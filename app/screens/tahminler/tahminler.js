...
export class Tahminler extends React.Component {
  static navigationOptions = {
    title: 'TAHMİNLER'.toUpperCase(),
  };
  ...
  componentWillMount() {
   ...
   let self=this;
        ...
        axios.get('https://eduasportin.com//json/listPredictionLeague.js?r='+Math.random())
...
renderTab = (isSelected, title,first=false) => {
  ...
  _renderItem(info) {
    if(info.item.ID=="topGal"){
        return(
          ...
          <RkTabView  rkType='rounded' >
              ...
              <RkTabView.Tab title={(selected) => {
                return this.renderTab(selected, '01/04');
              }}/>
              <RkTabView.Tab title={(selected) => {
                return this.renderTab(selected, '02/04');
              }}/>
              <RkTabView.Tab title={(selected) => {
                return this.renderTab(selected, '03/04');
              }}/>
              <RkTabView.Tab title={(selected) => {
                return this.renderTab(selected, '04/04');
              }}/>
    ...
    return (
      ...
      <View>
        <TahminLig navigation={this.props.navigation} league={info.item.apiLeagueID} header={info.item.name}/> 
